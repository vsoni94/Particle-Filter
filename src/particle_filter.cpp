...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  // Setting the flag that the filter has been initialized.
  is_initialized = true; 
}
...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
  ...
  for (int i = 0; i < num_particles; ++i) {
    ...
    // Checking if yaw_rate is less than threshold
...
void ParticleFilter::dataAssociation(vector<LandmarkObs> predicted, 
  ...
  for (unsigned int i = 0; i < nObservations; ++i) { // For each observation
    ...
    // Setting initial distance as a high number
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for (int i = 0; i < num_particles; ++i) {
    ...
    double theta = particles[i].theta;
    // vector to save locations of landmarks
    vector<LandmarkObs> inRangeLandmarks;
    for(unsigned int j = 0; j < map_landmarks.landmark_list.size(); ++j) {      
      // Getting ID and coordinates of landmark
    ...
    // Transforming observed coordinates.
